...
module.exports.run = async (client,message,args) =>{
    ...
    if (arg.length!=2){
        message.channel.send({embed:{color:0xFFA500, description:"Команда введена неверно. %agi <количество характеристик>"}})
    }else{
        if(parseInt(arg.slice(1))>p.stat){
            message.channel.send({embed:{color:0xFFA500, description:"у вас недостаточно характеристик"}})
        }else{
            p.agi=p.agi+parseInt(arg.slice(1))
            p.stat=p.stat-parseInt(arg.slice(1))
            fs.writeFile("./profile.json", JSON.stringify(profile), (err) => {
                if(err) console.log(err);
            })
            //message.channel.send("добавлено " +parseInt(arg.slice(1)) +" очков к ловкости")
            message.channel.send({embed: {
                color:0xFFA500,
                fields:[{
                  name:"Добавление характеристик",
                  value: "Ловкость повышена на "+parseInt(arg.slice(1))
               }],
            }})
        }
    }   
